...
export default function CatalogClient({ images }: { images: string[] }) {
  ...
  return <main className="catalog-page">
    ...
    <Footer />
    {selected && selectedIndex >= 0 && <ImageModal previousSrc={lang === "en" ? `/images/en/${imageName(filtered[(selectedIndex - 1 + filtered.length) % filtered.length])}.webp` : filtered[(selectedIndex - 1 + filtered.length) % filtered.length]} nextSrc={lang === "en" ? `/images/en/${imageName(filtered[(selectedIndex + 1) % filtered.length])}.webp` : filtered[(selectedIndex + 1) % filtered.length]} src={lang === "en" ? `/images/en/${imageName(selected)}.webp` : selected} alt={imageName(selected)} onClose={closePreview} onPrevious={previousPreview} onNext={nextPreview} currentPosition={selectedIndex + 1} total={filtered.length}/>}
  </main>;
}
